  ...
  const descargarPDFTipo=async()=>{
    try {
      if (tipo==='') {
        ...
        const url = `https://backendalertapj.gongalsoft.com/api/reporte/alerta/${tipo}`;
  ...
  const descargarPDFInformatico=async()=>{
    try {
      if (tipo==='') {
        ...
        const url = `https://backendalertapj.gongalsoft.com/reporte/alertaderivada/${informatico}`;
        await Linking.openURL(url);
      }
...
export default ReportSupervisorScreen;
